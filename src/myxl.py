...
class myxl(object):
	...
	def get_default_subscriber_number(self):
		...
		host = 'my.xl.co.id'
		response = self.request('POST', f"https://{host}/pre/PayloadQueryBalanceReq",
			json={
				"Header": None,
				"Body": {
					"Header": {
		...
		subscriber_number = (
			response.get('SOAP-ENV:Envelope', {})
				.get('SOAP-ENV:Body', {})[0]
				.get('BilDiameterMediation:PayloadQueryBalanceResp', {})[0]
				.get('diabilling:QueryInformation', {})[0]
				.get('diabilling:SubscriberID', {})[0]
		)
	...
	def buy_package(self, data):
		while self.loop:
			...
			msisdn = self.msisdn if not self.gift_to else self.gift_to
			platform = str(data['platform'])
			service_id = str(data['service_id'])
			subscriber_number = str(data['subscriber_number'])
